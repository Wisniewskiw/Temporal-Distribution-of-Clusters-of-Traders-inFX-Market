...
class AA(object):
    ...
    def dapra_loader(self):
        assert  self.cluster_cut in [1000, 500]

        if self.dapra_howmanyrows<1:
            self.dapra = pd.read_csv(self.dapra_path,
                                     parse_dates=['QdfTime'],
                                     infer_datetime_format=True).drop('Unnamed: 0', axis=1)
        else:
            self.dapra = pd.read_csv(self.dapra_path,
                                     parse_dates=['QdfTime'],
                                     nrows=self.dapra_howmanyrows,
                                    infer_datetime_format=True).drop('Unnamed: 0', axis=1)
    ...
    def cluster_loader(self,fraction=0.5):
        assert self.cluster_cut in [1000, 500]

        if self.cluster_is_dynamic == 0:
            s = 'static_' + str(self.cluster_static) + '_' + self.cluster_window
            s2 = 'clusters_start_end_dates_static' + str(self.cluster_static) + \
                 '_cut_' + str(self.cluster_cut) + '_' + self.cluster_window

        else:
            s =  self.cluster_window + "_EURUSD"
            s2 = 'clusters_start_end_dates_dynamic_cut_' + str(self.cluster_cut) +\
                 '_' + self.cluster_window

        self.clusters =  cluster_dynamics2(s,
        ...
        self.cluster_start_end_dates_df = pd.read_csv(self.cluster_dates_path + s2 + '_cut.csv',
                                                      parse_dates=['start','shift_start','end'],
        ...
        #if  'year' in self.cluster_window:
        #    self.cluster_start_end_dates_df['shift_start'].iloc[-1] = self.cluster_start_end_dates_df['end'].iloc[
        #                                                                  -1] + np.timedelta64(30, 'D')
        #else:
        #    self.cluster_start_end_dates_df['shift_start'].iloc[-1]=self.cluster_start_end_dates_df['end'].iloc[-1] \
        #                                                           + np.timedelta64(14, 'D')

        ##LEAK
        self.cluster_start_end_dates_df['shift_start']=self.cluster_start_end_dates_df['start'].shift(-1)

        if  'year' in self.cluster_window:
            self.cluster_start_end_dates_df['shift_start'].iloc[-1] = self.cluster_start_end_dates_df.start.iloc[
                                                                          -1] + np.timedelta64(30, 'D')
        else:
            self.cluster_start_end_dates_df['shift_start'].iloc[-1]=self.cluster_start_end_dates_df.start.iloc[-1] + np.timedelta64(14, 'D')
    ...
    def dates_leak(self):
        ...
        #LEAK
        self.dapra = self.dapra[(self.dapra.QdfTime >= self.cluster_start_end_dates_df['start'].values[0])
                                & ((self.dapra.QdfTime < self.cluster_start_end_dates_df['shift_start'].values[-1]))]
    ...
    def LS_loss(self,c,eps=0):
        ...
        if self.aa_lr==1:
            ...
            tmp = -np.log(tmp0).replace(np.nan,np.inf)

            return np.exp(-self.aa_lr*tmp)
    ...
    def LSD_loss(self,c,eps=0):
        ...
        if self.aa_lr==1:
            ...
            return np.exp(self.aa_lr*np.log(tmp))
    ...
    def LSN_loss(self,c,eps=0):
        ...
        if self.aa_lr==1:
            ...
            return  np.exp(self.aa_lr*np.log( np.maximum(1+tmp1+tmp2,0)))


    def calculate_loss(self):
            cols=[x for x in  self.dapra.columns if ('_' not in x) and ('Qdf' not in x)   ]
            if self.aa_losstype=='LS':
                self.dapra[cols]=self.LS_loss(self.dapra[cols] ).values
            if self.aa_losstype=='LSD':
                self.dapra[cols]=self.LSD_loss(self.dapra[cols] ).values
            if self.aa_losstype=='LSN':
                self.dapra[cols]=self.LSN_loss(self.dapra[cols] ).values


    def calculate_lossCAA2(self,df):

            if self.aa_losstype=='LS':
                return self.LS_loss(df ).values
            if self.aa_losstype=='LSD':
                return self.LSD_loss(df ).values
            if self.aa_losstype=='LSN':
                return self.LSN_loss(df ).values
    ...
    def AA_return(self):
        cols = [x for x in self.dapra.columns if ('_' not in x) and ('Qdf' not in x)]  # loss
        cols2 = [x for x in self.dapra.columns if 'omega' in x]  # active or not
        cols3 = [x for x in self.dapra.columns if 'gamma' in x]  # gamma
        ...
        res, omegaAA, r = calculator_probas_sleeping2(self.dapra[cols3].values, self.dapra[cols].values,
                                                          ...
                                                          self.aa_rho, self.aa_rhobis, self.aa_u, self.aa_ubis)
    ...
    def gammaCAA(self):
        ...
        for ind, row in (self.cluster_start_end_dates_df.iterrows()):
            ...
            n = [len(x) for x in s]
            k = [list(np.repeat(x, x)) for x in n]
            n=[x for y in k for x in y]

            s = [x + "_gamma" for y in s for x in y]
            ...
            o = pd.DataFrame(np.ones(tmp.shape), columns=tmp.columns)
            x = o.iloc[0, :].copy()

            #o = pd.DataFrame(np.zeros(tmp.shape), columns=tmp.columns)
            #x = o.iloc[0, :].copy()
            ...
            self.dapra.loc[tmp.index,cols] =(tmp.mul(o))
    ...
    def gammaCAA2(self):
        ...
        for i in range(len(self.clusters)):
            ...
            res, gammaAA, r = grouped_calculator_probas_sleeping(gamma.values, loss ,
                                                            ...
                                                            self.aa_lr, self.aa_rho, self.aa_rhobis, self.aa_u, self.aa_ubis)
    ...
    def save(self,s):
        ...
        if self.aa_losstype =='LSN':
            s=s+'_LSN_'+str(self.aa_lr)+'_'+str(self.aa_rho)+'_'+str(self.aa_rhobis)+'_'+str(self.aa_u)+'_'+str(self.aa_ubis)+'_'
        else:
            s=s+'_'+self.aa_losstype+'_'+str(self.aa_lr)+'_'+str(self.aa_rho)+'_'

        s3='_CL_algo_'+ self.cluster_algo+  '_CL_res_'+str(self.cluster_resolution)+'_CL_tr_'+str(self.cluster_truncate_cluster_cardinality)
    ...
    def run_paper_scaling_factor(self,lossname,max_scaling,cluster_resolution=0):
        ...
        saveto="_scalling_fullsleeping_"+str(self.full_seeping)+"_loss_" +lossname+"_clusterresolution_" +str(cluster_resolution)+"_cutoff_"+str(self.cluster_cut)
...
def plot_scaling(lossname, res):
    mypath = "."

    onlyfiles = [f for f in listdir(mypath) if isfile(join(mypath, f))]

    result = [f for f in onlyfiles if '_' + lossname + '_' in f and '_' + str(res) + '_' in f]
    s = [x for x in result if 'sharpe' in x]
    r = [x for x in result if 'return' in x]
