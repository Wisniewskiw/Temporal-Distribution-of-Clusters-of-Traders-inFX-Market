...
class AA(object):
    ...
    def run_paper_scaling_factor(self, lossname, max_scaling, cluster_resolution=0):
        ...
        for i, n in enumerate(sh.columns):
            fig.add_scatter(x=sh.index, y=sh[sh.columns[i]], name=sh.columns[i], row=1, col=1,mode='lines', line=dict(color=colors[i]))
        ...
        for i, n in enumerate(re.columns):
            fig.add_scatter(x=re.index, y=re[re.columns[i]], name=re.columns[i], row=1, col=2,mode='lines', line=dict(color=colors[i]))
        ...
        # fig = px.line()
        for i, n in enumerate(re.columns):
            fig.add_scatter(x=sr.index, y=sr[sr.columns[i]], name=sr.columns[i], row=1, col=3,mode='lines', line=dict(color=colors[i]))
        ...
        for i, n in enumerate(dd_.columns):
            fig.add_scatter(x=dd_.index, y=dd_[dd_.columns[i]], name=dd_.columns[i], row=1, col=4,mode='lines', line=dict(color=colors[i]))
        ...
        # fig = px.line()
        for i, n in enumerate(dd_.columns):
            fig.add_scatter(x=ddd.index, y=ddd[ddd.columns[i]], name=ddd.columns[i], row=1, col=5,mode='lines', line=dict(color=colors[i]))
        ...
        saveto = "leak_nottruemean_scalling_fullsleeping_" + str(
